...
public class TraineeDAO {
	...
	public StudentVO getStudentSubjectName(String sd_id) throws Exception {
		...
		try {
			...
			pstmt.setString(2, sd_id); // 이유를 모르겠음
	...
	public String getLessonNum(String lessonName) throws Exception {
		...
		try {
			...
			if (rs.next()) {
				...
				alert.setHeaderText("선택한 " + lessonName + " 과목의 번호가 없습니다.");
	...
	// 수강신청
	public void getTraineeResiste(TraineeVO tvo) throws Exception {
		...
		try {
			...
			if (i == 1) {
				...
				alert.setTitle("수강신청");
				alert.setHeaderText("수강신청 완료.");
				alert.setContentText("수강신청 성공!!!");
				alert.showAndWait();
				...
				alert.setHeaderText("수강신청 실패");
				alert.setContentText("수강신청 실패!");
				alert.showAndWait();
