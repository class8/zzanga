...
public class MainController implements Initializable {
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		try {
			...
			// 메뉴 이벤트 등록
			menuExit.setOnAction(event -> handlerMenuExitAciton(event));
	...
	public void handlerMenuExitAciton(ActionEvent event) {

		Alert alert;
		alert = new Alert(AlertType.INFORMATION);
		alert.setTitle("미래 대학교");
		alert.setHeaderText("미래 대학교 수강신청 프로그램 종료");
		alert.setContentText("확인 버튼을 클릭하면 미래 대학교 수강신청 프로그램을 종료합니다.");
		alert.setResizable(false);
		alert.showAndWait();
		Platform.exit();
